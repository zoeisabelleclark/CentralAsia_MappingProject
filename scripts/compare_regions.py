...
CENSUS_FILE = Path("data/interim/kazakhstan_census_long.csv")
BOUNDARY_FILE = Path("data/interim/kazakhstan_regions.json")
...
def main():
    ...
    # Differences
    census_only = sorted(set(census_regions) - set(boundary_regions))
    boundary_only = sorted(set(boundary_regions) - set(census_regions))

    print("\n=== IN CENSUS BUT NOT IN BOUNDARIES ===")
    for r in census_only:
        print("-", r)

    print("\n=== IN BOUNDARIES BUT NOT IN CENSUS ===")
    for r in boundary_only:
        print("-", r)
