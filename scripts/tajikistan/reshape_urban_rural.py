...
INFILE = Path("data/raw/tajikistan/census/urban_rural.csv")
OUTFILE = Path("data/interim/tajikistan/urban_rural_long.csv")
...
def main():
    ...
    national = df[df["region_name"] == "Republic of Tajikistan"].copy()
    national.to_csv("data/interim/tajikistan/tajikistan_national_urban_rural.csv", index=False)

    # Keep only regional rows
    df = df[df["region_name"] != "Republic of Tajikistan"].copy()
